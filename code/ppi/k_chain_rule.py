...
Original file is located at
    https://colab.research.google.com/drive/1bQMfw8IADaqVbxa87-dxPsByd1gn7w8m
"""

# kbin_cre_sensitivity_colab.py
# -*- coding: utf-8 -*-
"""
K-bin Bayesian CRE sensitivity (K in {2,4,5}) — Colab-ready
- Input CSV in Google Drive: MyDrive/age_analysis/autorater_predictions_all4.csv
- Output directory: MyDrive/K-bin/
- Reports posterior mean/95% CI for g = sum_k P(B=k)*P(H=1|B=k)
- Saves: kbin_sensitivity_summary.csv, fig_kbin_width_vs_K.png
"""
...
# 0) Paths & parameters
# ============================
# 입력 CSV는 age_analysis 폴더에서 읽음
CSV_PRED = GDRIVE / "age_analysis" / "autorater_predictions_all4.csv"  # must include 'autorater_prediction' and 'H'

# 출력은 새 폴더 K-bin 에 저장
OUT_DIR  = GDRIVE / "K-bin"
...
pred = pred[(pred['autorater_prediction'].between(0,1, inclusive='both')) & (pred['H'].isin([0,1]))].copy()

# Labeled subset (H available) and full pool for scores
use = pred.dropna(subset=['autorater_prediction','H']).copy()

if len(use) == 0:
    raise ValueError("No valid labeled rows found (check 'autorater_prediction' and 'H').")

# ============================
# 2) K-bin utilities
# ============================
def _assign_bins(scores: pd.Series, K: int, strategy: Literal["quantile","uniform"]="quantile") -> pd.Series:
...
def _posterior_draws_kbin(
    ...
    a_beta: float = 1.0, b_beta: float = 1.0,
    S: int = 5000,
    ...
    Returns dict with summary stats (mean, sd, ci_low, ci_high, width, K_effective).
    """
    # 1) Bin assignment on the full pool (for stable P(B))
    pred_valid = pred_df.dropna(subset=["autorater_prediction"]).copy()
    pred_valid["bin"] = _assign_bins(pred_valid["autorater_prediction"], K, strategy=strategy)
    bins_used = int(pred_valid["bin"].nunique())
    if bins_used < 2:
        raise ValueError(f"Effective number of bins is {bins_used} (<2). Try 'uniform' or smaller K.")
    ...
    lab["bin"] = _assign_bins(lab["autorater_prediction"], bins_used, strategy=strategy)
    ...
    return {
        ...
        "mean": mean, "sd": sd, "ci_low": float(lo), "ci_high": float(hi), "width": width
    }

def run_kbin_sensitivity(K_list=(2,4,5), strategy="quantile",
                         alpha_dirichlet=1.0, a_beta=1.0, b_beta=1.0,
                         S=5000, out_dir=OUT_DIR, rng=RNG):
    """
    Runs K-bin CRE for K in K_list; saves CSV + width-vs-K plot.
    """
    results = []
    for K in K_list:
        res = _posterior_draws_kbin(
            pred_df=pred, labeled_df=use, K=K, strategy=strategy,
            alpha_dirichlet=alpha_dirichlet, a_beta=a_beta, b_beta=b_beta,
            S=S, rng=rng
        )
        ...
        res["b_beta"] = b_beta
        results.append(res)
        print(f"[K-bin] K={K} (eff={res['K_effective']}): mean={res['mean']:.4f}, "
              f"95%CI=[{res['ci_low']:.4f},{res['ci_high']:.4f}] (width={res['width']:.4f})")

    df = pd.DataFrame(results)[
        ["K_requested","K_effective","strategy","mean","sd","ci_low","ci_high","width",
         "alpha_dirichlet","a_beta","b_beta"]
    ]
    df.to_csv(out_dir/"kbin_sensitivity_summary.csv", index=False, encoding="utf-8-sig")
    ...
    plt.tight_layout()
    plt.savefig(out_dir/"fig_kbin_width_vs_K.png", dpi=200)
    plt.close()

    print("\n✅ Saved:")
    print(f"- {out_dir/'kbin_sensitivity_summary.csv'}")
    print(f"- {out_dir/'fig_kbin_width_vs_K.png'}")

# ============================
# 3) Run
# ============================
# 노트북 셀에서 실행 시 __name__ == "__main__" 이므로 자동 실행됨
if __name__ == "__main__":
    run_kbin_sensitivity(
        K_list=K_LIST,
        strategy=BINNING_STRATEGY,     # "quantile" or "uniform"
        alpha_dirichlet=ALPHA_DIR,     # 0.5 for Jeffreys-like
        a_beta=A_BETA, b_beta=B_BETA,  # 0.5/0.5 for Jeffreys-like
        S=N_DRAWS,
        out_dir=OUT_DIR,
        rng=RNG
    )