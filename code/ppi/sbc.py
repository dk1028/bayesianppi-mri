...
# --- repo-relative paths ---
# Assume repo root is two levels above code/ppi/sbc.py
REPO_ROOT  = Path(__file__).resolve().parents[2]
FIGS_ROOT  = REPO_ROOT / "figs" / "sbc"   # dedicated folder for SBC figures
...
def run_and_save(prior="jeffreys", M=500, S=1000, NA=1000, NH=100,
                 B=20, outdir=None, seed=2025):
    """
    outdir:
      - None     -> use REPO_ROOT/figs/sbc
      - relative -> interpreted relative to REPO_ROOT/figs
      - absolute -> used as-is
    """
    if outdir is None:
        outdir_path = FIGS_ROOT
    else:
        outdir_path = Path(outdir)
        if not outdir_path.is_absolute():
            # If relative, place it under figs/
            outdir_path = FIGS_ROOT.parent / outdir_path
    ...
    fA  = outdir_path / f"sbc_thetaA_{suffix}.png"
    fH1 = outdir_path / f"sbc_thetaH1_{suffix}.png"
    fH0 = outdir_path / f"sbc_thetaH0_{suffix}.png"
    fG  = outdir_path / f"sbc_g_{suffix}.png"
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description="SBC rank histograms for conjugate CRE.",
        add_help=True
    )
    ...
    # If default=None, use REPO_ROOT/figs/sbc
...
def main(argv=None):
    ...
    if args.both:
        files_u = run_and_save(
            prior="uniform", M=args.M, S=args.S, NA=args.NA, NH=args.NH,
            B=args.bins, outdir=args.outdir, seed=args.seed
        )
        files_j = run_and_save(
            prior="jeffreys", M=args.M, S=args.S, NA=args.NA, NH=args.NH,
            B=args.bins, outdir=args.outdir, seed=args.seed
        )
        ...
        files = run_and_save(
            prior=args.prior, M=args.M, S=args.S, NA=args.NA, NH=args.NH,
            B=args.bins, outdir=args.outdir, seed=args.seed
        )
